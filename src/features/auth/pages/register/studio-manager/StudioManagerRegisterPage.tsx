...
const STEPS = [
  ...
  { key: "details", title: "Role Details", subtitle: "Your responsibilities" },
...
interface StudioManagerRegisterPageProps {
  ...
  studioName?: string;
  onBack: () => void;
  onSubmitted: (data: StudioManagerFormData) => void;
}

const EMPTY_DATA: StudioManagerFormData = {
  basicInfo: {
    firstName: "",
    lastName: "",
    phone: "",
  ...
  managerDetails: { department: "", responsibilities: "", yearsExperience: "" },
};
...
export default function StudioManagerRegisterPage({
  ...
  const [formData, setFormData] = useState<StudioManagerFormData>(EMPTY_DATA);
  ...
  const updateBasicInfo = (patch: Partial<StudioManagerFormData["basicInfo"]>) =>
    setFormData((prev) => ({ ...prev, basicInfo: { ...prev.basicInfo, ...patch } }));

  const updateKyc = (patch: Partial<StudioManagerFormData["kyc"]>) =>
    setFormData((prev) => ({ ...prev, kyc: { ...prev.kyc, ...patch } }));

  const updateManagerDetails = (patch: Partial<StudioManagerFormData["managerDetails"]>) =>
    setFormData((prev) => ({ ...prev, managerDetails: { ...prev.managerDetails, ...patch } }));
  ...
  return (
    <div className="studio-register-root">
      <div className="studio-register-header">
        <h1 className="studio-register-title">Join as Studio Manager</h1>
        ...
        <AnimatePresence mode="wait">
          {activeStep === "basic" ? (
            ...
            <motion.div key="details" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
              <div className="studio-form-section-header">
                <h2 className="studio-form-section-title">Role Details</h2>
                <p className="studio-form-section-subtitle">A little about your role at the studio</p>
              </div>
              <div className="studio-form-divider" />

              <div className="studio-form-grid">
                <div className="studio-form-field">
                  <label className="studio-field-label">Department</label>
                  <input className="studio-input" value={formData.managerDetails.department} onChange={(e) => updateManagerDetails({ department: e.target.value })} placeholder="e.g. Operations" />
                </div>
                <div className="studio-form-field">
                  <label className="studio-field-label">Years of Experience</label>
                  <input className="studio-input" value={formData.managerDetails.yearsExperience} onChange={(e) => updateManagerDetails({ yearsExperience: e.target.value })} placeholder="e.g. 1–3 years" />
                </div>
                <div className="studio-form-field studio-form-field--full">
                  <label className="studio-field-label">Responsibilities</label>
                  <textarea className="studio-textarea" value={formData.managerDetails.responsibilities} onChange={(e) => updateManagerDetails({ responsibilities: e.target.value })} placeholder="What will you be managing day to day?" />
                </div>
              </div>
            ...
            <motion.div key="review" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
              ...
              <div className="studio-form-grid">
                ...
                <div className="studio-form-field studio-form-field--full">
                  <label className="studio-field-label">Department / Responsibilities</label>
                  <span>{formData.managerDetails.department || "—"} — {formData.managerDetails.responsibilities || "No details provided"}</span>
                </div>
              </div>
