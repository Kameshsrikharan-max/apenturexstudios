...
const statusMeta = {
  DRAFT: { icon: <EditOutlined />, color: "#cbd5e1", bg: "rgba(148, 163, 184, 0.14)" },
  PLANNED: { icon: <ClockCircleOutlined />, color: "#93c5fd", bg: "rgba(59, 130, 246, 0.14)" },
  LIVE: { icon: <ThunderboltOutlined />, color: "#86efac", bg: "rgba(34, 197, 94, 0.14)" },
  DONE: { icon: <CheckCircleOutlined />, color: "#c4b5fd", bg: "rgba(124, 58, 237, 0.14)" },
};
...
export default function EventPage() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      const matchesSearch =
        !term || Object.values(event).some((value) => String(value).toLowerCase().includes(term));
    ...
    return String(value)
      ...
      .map((part, index) =>
        part.toLowerCase() === searchTerm.trim().toLowerCase() ? (
          <mark className="event-search-highlight" key={`${part}-${index}`}>
            {part}
          </mark>
        ) : (
          part
  ...
  const handleSave = (values) => {
    const cleanValues = { ...values, members: Number(values.members || 0) };

    if (editEvent) {
      setEvents((prev) =>
        prev.map((event) => (event.id === editEvent.id ? { ...event, ...cleanValues } : event))
      );
  ...
  const renderStatus = (status) => {
    ...
    return (
      <Tag className="event-status-tag" style={{ "--tag-color": meta.color, "--tag-bg": meta.bg }}>
        {meta.icon}
        {status}
      </Tag>
    );
  ...
  const columns = [
    {
      ...
      width: 190,
      render: (text, record) => (
        <button type="button" className="event-name-cell" onClick={() => setViewEvent(record)}>
    ...
    },
    {
      ...
      width: 128,
      render: (text) => <span className="event-soft-cell">{highlightText(text)}</span>,
    },
    {
      title: "Time",
      dataIndex: "time",
      ...
      <main className="event-page">
        <section className="event-hero">
          <div className="event-hero-copy">
            ...
            <Text>
              Plan bookings, customers, city, stage and assigned members in one transparent workspace.
            </Text>
            <div className="event-hero-actions">
              <Button type="primary" icon={<PlusOutlined />} className="event-hero-create" onClick={openCreate}>
                Create Event
              </Button>
          ...
          <div className="event-hero-art" aria-hidden="true">
            {initialEvents.slice(0, 3).map((event, index) => (
              <img
                ...
                src={event.image}
                alt=""
                className={`event-hero-photo photo-${index + 1}`}
              />
            ))}
        ...
        <section className="event-panel">
          <div className="event-toolbar">
            <Space size={10} wrap>
              ...
              >
                <Tooltip title="Filter">
                  <Button type="text" icon={<FilterOutlined />} className="event-tool-btn" />
                </Tooltip>
              </Popover>
            ...
            <Button type="primary" icon={<PlusOutlined />} className="event-create-btn" onClick={openCreate}>
              Create Event
            </Button>
          </div>
          ...
          <div className="event-chip-row">
            {["All", ...statuses].map((status) => (
              <Tooltip title={status} key={status}>
                <button
                  type="button"
                  className={`event-status-chip ${activeStatus === status ? "active" : ""}`}
          ...
          {viewMode === "table" ? (
            <Table
              ...
              scroll={{ x: 1380 }}
              locale={{
                emptyText: <Empty description="No matching events" image={Empty.PRESENTED_IMAGE_SIMPLE} />,
              }}
            />
          ) : (
                ...
                <article className="event-card" key={event.id}>
                  ...
                  <div className="event-card-body">
                    ...
                    <div className="event-card-footer">
                      ...
                      <Space size={4}>
                        <Tooltip title="Assign members">
                          <Button type="text" icon={<TeamOutlined />} onClick={() => openEdit(event)} />
                        </Tooltip>
                        <Tooltip title="Copy to clipboard">
                          <Button type="text" icon={<CopyOutlined />} onClick={() => copyEventToClipboard(event)} />
                        </Tooltip>
                      </Space>
                    </div>
                  </div>
          ...
          {viewEvent && (
            <div className="event-view-shell">
              ...
              <div className="event-view-grid">
                <InfoTile icon={<CalendarOutlined />} label="Date" value={viewEvent.date} />
                <InfoTile icon={<ClockCircleOutlined />} label="Time" value={viewEvent.time} />
                <InfoTile icon={<EnvironmentOutlined />} label="City" value={viewEvent.city} />
                <InfoTile icon={<TeamOutlined />} label="Members" value={viewEvent.members} />
              </div>
        ...
        >
          <div className="event-modal-shell">
            <div className="event-modal-heading">
              <Avatar className="event-modal-avatar">{editEvent ? <EditOutlined /> : <PlusOutlined />}</Avatar>
              <div>
                <Title level={3}>{editEvent ? "Edit Event" : "Create Event"}</Title>
                <Text>Fill the event details.</Text>
              </div>
            </div>
            ...
            <Form form={form} layout="vertical" onFinish={handleSave}>
              <div className="event-form-grid">
                ...
                <Form.Item name="type" label="Event Type">
                  <Select options={eventTypes.map((type) => ({ value: type, label: type }))} />
                </Form.Item>
                ...
                <Form.Item name="pipeline" label="Stage">
                  <Select
                    options={["Converted", "Booked", "Proposal", "Delivered"].map((stage) => ({
                      value: stage,
                      label: stage,
                    }))}
                  />
                ...
                <Form.Item name="address" label="Address" rules={[{ required: true }]} className="event-form-wide">
                  <Input placeholder="mettupalayam" />
                </Form.Item>
                <Form.Item name="image" label="Image URL" className="event-form-wide">
              ...
              <div className="event-modal-actions">
                ...
                <Button type="primary" htmlType="submit" icon={editEvent ? <EditOutlined /> : <PlusOutlined />}>
                  {editEvent ? "Save Event" : "Create Event"}
                </Button>
              </div>
            </Form>
          </div>
